...
export class UserWorkRateComponent implements OnInit, OnChanges {
  ...
  newSchedule(datevalue: Date) {
    ...
    this.selDateValue = hf.handleDate(datevalue);
    this.viewBulk();
  }
  viewBulk() {
    if (!this.selDateValue) {
      return;
    }
    this.ngOnInit();
    this.srvDash
      .getUserVisitRate(this.dispUser.userID, this.selDateValue)
      .subscribe(
        rat => {
          ...
          this.chartData = [
            {
              data: rat[0].map(da => {
                return da.visCount == null ? 0 : da.visCount;
              }),
              label: 'Visit Count'
            }
          ];
          this.lineChartLabels = rat[0].map(data => {
            return data.DayDate.split('T')[0];
          });
